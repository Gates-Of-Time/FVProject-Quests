sub EVENT_SAY {
	if ($text=~/Hail/i) {
		quest::say("What?  What do you want. furless?  My tseq, Graalf, lies sick and you waste time with talk when you [" . quest::saylink("cannot help") . "].");
	}
	if ($text=~/cannot help/i) {
		quest::say("The shamans, they have said I need [" . quest::saylink("talismans") . "] to ward the sickness from Graalf.");
	}
	if ($text=~/talismans/i) {

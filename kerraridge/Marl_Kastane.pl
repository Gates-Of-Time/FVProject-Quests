sub EVENT_SAY {
	...
	if ($text=~/Hail/i) {
	...
	elsif ($text=~/prophecy/i && $faction >= 42 && $faction < 44) {
		...
		#:: Give a 14375 - Seal of Kastane
      	quest::summonitem(14375);
	}
	elsif ($text=~/prophecy/i && $faction >= 44) {
	...
	elsif ($text=~/Lhranc/i && $faction >= 44) {
	...
	elsif ($text=~/prediction/i && $faction >= 44) {
	...
	elsif ($text=~/secret plan/i && $faction >= 44) {
	...
	elsif ($text=~/I will complete the ritual/i && $faction >= 44) {
...
sub EVENT_ITEM {
	$faction = $client->GetCharacterFactionLevel(404);	#:: Truespirit (404)
	#:: Match a 14376 - Note to Marl
	if ($faction >= 43 && plugin::takeItems(14376 => 1)) {
		...
		#:: Set factions
		quest::faction(404, 1);		#:: + True Spirit
		#:: Ding!
		quest::ding();
	}
	#:: Match a 14381 - Blood of Kyrenna
	elsif ($faction >= 51 && plugin::takeItems(14381 => 1)) {
	...
	#:: Match a 14379 - Glohnor wrappings
	elsif ($faction >= 58 && plugin::takeItems(14379 => 1)) {

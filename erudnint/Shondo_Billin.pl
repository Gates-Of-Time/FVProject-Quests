sub EVENT_SAY {
	if ($text=~/hail/i) {
		quest::say("Oh, Brell!! Help me get out of here. I did nothing wrong!! I did not know you could not take that [" . quest::saylink("tonic") . "] out of the city.");
	}
	if ($text=~/tonic/i) {
...
sub EVENT_ITEM {
	...
	if (plugin::check_handin(\%itemcount, 13126 => 1)) {
		...
		quest::exp(100);
		#:: Set Factions
		quest::faction(115,10);		#:: Gem Choppers +10

sub EVENT_SAY {
  ...
  if ($faction <= 2) {
    if ($text =~ /hail/i) {
      ...
      #:: Give a 1725 - Velium Delivery Note
      quest::summonitem(1725);
    }
    #:: Require the friendofthekin global
    elsif (($friendofthekin == 1) && ($text =~ /i am interested/i)) {
    ...
    #:: Require the friendofthekin global
    elsif (($friendofthekin == 1) && ($text =~ /source/i)) {
    ...
    #:: Require the friendofthekin global
    elsif (($friendofthekin == 1) && ($text =~ /i am capable/i)) {
      ...
      #:: Give a 29068 - Note to Wenglawks
...
sub EVENT_ITEM {
  #:: Match a 29064 - Shipment of Velium Ore
  if (plugin::takeItems(29064 => 1)) {
    ...
    #:: Give a 1727 - Bracer of Hammerfal
  ...
  #:: Require the friendofthekin global and match a 29065 - Spy Report
  elsif (($friendofthekin == 1) && plugin::takeItems(29065 => 1)) {
    quest::say("You have shown us yet again your loyalty to our people. Please accept this token of our gratitude.");
    #:: Give a 29050 - Dragon Tooth Choker

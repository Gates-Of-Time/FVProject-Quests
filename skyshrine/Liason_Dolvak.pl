sub EVENT_SAY {
	if ($text =~ /hail/i) {
		...
		#:: Give a 29067 - Note to Tunare's Court
		quest::summonitem(29067);
	}
}

sub EVENT_ITEM {
	#:: Match a 29623 - Eysa's Token
	if (plugin::takeItems(29623 => 1)) {
		...
		quest::exp(5000);
		#:: Give cash reward: 2 copper, 3 silver, 5 gold, 7 platinum

sub EVENT_SAY {
  if ($faction == 1) {
    if ($text =~ /hail/i) {
    ...
    elsif ($text =~ /ancient dragon tome/i) {
      ...
      #:: Give a 17166 - Empty Dragon Tome Binding
...
sub EVENT_ITEM {
  if ($faction == 1) {
    #:: Match a 12550 - Complete Dragon Tome
    if (plugin::takeItems(12550 => 1)) {
      ...
      #:: Give a 12558 - Orb of Draconic Energy

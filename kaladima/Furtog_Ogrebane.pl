...
sub EVENT_SAY { 
	if ($text=~/hail/i) {
		quest::say("Hail. mighty $name! Welcome to Stormguard Hall. I am Captain Furtog Ogrebane, only surviving descendant of the legendary Trondle Ogrebane, slayer of the [" . quest::saylink("mudtoes") . "]. If you wish to serve the grand city of Kaladim, I urge you to speak with any of the trainers. Good day.");
	}
	if ($text=~/mudtoe/i) {
		quest::say("The Mudtoes were a small but mighty clan of ogres. My great father [" . quest::saylink("Trondle") . "] destroyed them. I have heard disturbing rumors of one Mudtoe surviving. Would you mind [" . quest::saylink("searching") . "] for the Mudtoes or have you other duties to perform?");
	}
	if ($text=~/trondle/i) {
	...
	if ($text=~/searching/i) {
		quest::say("I have heard persistent rumors of two Mudtoe ogres who survived the slaughter.  Go to the port of Butcherblock.  Those dock hands must have heard of the name [" . quest::saylink("Mudtoe") . "].  Bring me the remaining heads of the Mudtoes!!  I want all the Mudtoes dead!!  Do not return until you have them both!!");
	}
	if ($text=~/trades/i) {

...
if($text=~/Hail/i) {
   ...
   if ($text=~/Jagged Fang]/i) {
     ...
     quest::spawn2(70071,0,0,-1416,-1486,52,13); # spawn Jagged Fang
   }
}
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 67003 =>1 )) { # Note to Grimpl
  ...
  if (plugin::check_handin(\%itemcount, 67005 =>1 )) { # Venom Filled Scales
    ...
    quest::summonitem(67005); # Note to Grimpl
    quest::summonitem(67004); # Note to Saeko
  }

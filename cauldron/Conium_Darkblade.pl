...
sub EVENT_ITEM {
	...
	if (($cash >= 3000) && plugin::check_handin(\%itemcount, 13260 => 3)) { # Giant Wasp Venom Sac
	  quest::say("Take the giant wasp venom. Good or evil it is not. That is up to you.");
      quest::summonitem(14032); # Regalis Poison
   }
   if (plugin::check_handin(\%itemcount, 18955 => 1)) { # Sealed Note
      ...
      quest::summonitem(12366); # Never Stop Chopping
   }

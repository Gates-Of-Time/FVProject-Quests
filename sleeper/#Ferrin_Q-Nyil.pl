...
sub EVENT_SIGNAL {
	#:: Match if signal from sleeper/#Bittles_the_Lost.pl is "0"
	if ($signal == 0) {
		quest::say("Prismatic, you goof.");
		#:: Send a signal "0" to Sleeper's Tomb >> Bittles the Lost (128036) with no delay
		quest::signalwith(128036, 0, 0);
	}
}

sub EVENT_ITEM {
	quest::say("I have no use for that, keep it.");
	#:: Return unused items

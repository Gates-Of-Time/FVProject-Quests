sub EVENT_SAY {
	if ($text=~/hail/i) {
		...
		#:: Send a signal "1" to Steamfont Mountains >> Charlotte (56108)
...
sub EVENT_WAYPOINT_ARRIVE {
	if ($wp==12) {
		#:: Send a signal "2" to Steamfont Mountains >> Charlotte (56108)
		quest::signalwith(56108,2,1);
	}
	if ($wp==18) {
		#:: Send a signal "3" to Steamfont Mountains >> Charlotte (56108)

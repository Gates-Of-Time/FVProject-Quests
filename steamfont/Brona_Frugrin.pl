sub EVENT_SAY { 
  if($text=~/Hail/i){
    quest::say("Well met, $name!  What brings you out this way?  Are you interested in becoming an observer?  No, you look like the adventuring type.  The wilds of the Steamfont Mountains is as far as my body goes.  But through my [" . quest::saylink("duties") . "] as an observer, my mind travels the cosmos.");
  }
  if($text=~/duties/i){
    quest::say("Well, I am quite an accomplished enchanter but most of my time now is spent crafting the magical lenses that enable us to see beyond the ceiling of Norrath and into other realms and dimensions.  Say, in your travels have you encountered any [" . quest::saylink("evil eyes") . "]?");
  }
  if($text=~/evil eyes/i){

sub EVENT_SAY { 
	if ($text=~/hail/i) {
		quest::say("Hey, hey, hey! Welcome to the Fool's Gold. Don't bet more than you can afford to lose, pal! You don't want to end up like ol' [" . quest::saylink("Kevlin") . "].");
	}
	elsif ($text=~/kevlin/i) {
		quest::say("Kevlin Diggs. His family sells armor here in Rivervale. He has a nice shop right across from us here. But he owes us Deeppockets some [" . quest::saylink("gold") . "] that we need to collect. We can't rough him up because he is a friend of Mayor Gubbin. We need someone to steal it.");
	}
	elsif ($text=~/gold/i) {

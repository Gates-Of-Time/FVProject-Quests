...
sub EVENT_ENTER {
  #:: Check for 18733 - Recruitment Summons
...
sub EVENT_SAY {
	if ($text=~/hail/i) {
		quest::say("Hail, $name! Stand tall whenever you are speaking to the Sheriff of Rivervale. I command the warriors of this vale. You must be new to the ranks of the Guardians of the Vale, so be sure to report to your squad at once. We must protect our people. The [" . quest::saylink("danger") . "] has come closer to home. If you are not a deputy, then please leave these halls.");
	}
	if ($text=~/danger/i) {
...
sub EVENT_ITEM {
	#:: Turn in for 18733 - Warrior Recruitment Letter
	...
	#:: Turn in for 13931 - Runnyeye Warbeads x4
	if (plugin::takeItems(13931 => 4 )) {
		...
		quest::givecash(0,8,0,0);	#:: Give a small amount of cash copper - plat
	}
	#:: Return unused items

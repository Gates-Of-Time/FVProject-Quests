...
sub EVENT_SAY { 
	if ($text=~/hail/i) {
		...
		quest::say("Hay, ho, there, young $race!  I'd love to yak it up with you but I'm a bit busy at the moment, trying to find some [" . quest::saylink("help") . "].");
	}
	if ($text=~/help/i) {
		quest::say("Well, we're experimenting with some tanning methods but we're running low on supplies. We need to find someone to go out and [" . quest::saylink("gather some things") . "]. Seems no one wants to do an honest day's work any more. I'd do it myself but, errrr, my, uhhh, foot hair has been hurting lately... yes, that's it.");
	}

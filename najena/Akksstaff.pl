sub EVENT_SAY {
	...
	if ((!defined $qglobals{"ask_minion_dead"}) && $text=~/hail/i ) {
	...
	} elsif ($text=~/bantil sent me/i) && $qglobals{"mage_epic"} >= "5") {
...
sub EVENT_ITEM {
	...
	if (plugin::takeItems(28027 => 1, 28028 => 1, 28029 => 1, 28030 => 1)) {
		quest::say("Very well. Take these words back to that shriveled old gnome, Rykas, if you wish to complete this quest. Now leave me, " .. e.other:GetName() .. ", it sickens me to be this cordial for so long. And my pet needs exercise. Go!");
		#:: Ding!
		quest::ding();

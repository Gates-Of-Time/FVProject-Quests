sub EVENT_SAY {
	if ($text=~/hail/i) {
		quest::say("Hail, Adventurer! I hope you are enjoying your time in Surefall Glade. You must be careful when leaving the Jaggedpine. There have been many report of [" . quest::saylink("Sabertooths") . "] attacking travelers.");
	}
	elsif ($text=~/sabertooths/i) {
		quest::say("The gnolls of Blackburrow are called Sabertooths. They have been seen in force on a regular basis. They are surely up to something. We have even heard rumors of a [" . quest::saylink("pact") . "] between Qeynos merchants and the dogs.");
	}
	elsif ($text=~/pact/i) {
		quest::say("Sources have come forward to tell of a pact between a merchant of Qeynos and the Sabertooths. It all has to do with blades and brew. We hear that a meeting will occur soon, somewhere in the Qeynos Hills at night. Do not fear, we shall find a brave ranger to [" . quest::saylink("halt this meeting") . "].");
	}
...
sub EVENT_ITEM {
	...
	if (plugin::takeItems(13309 => 1)) {
		quest::say("So, I see you rid the hills of the beast. Good work! I have a reward for you. I hope it will be usefull. I am afraid this gnoll's death will not halt the alliance between the two. I shall require your services to [" . quest::saylink("assist in the extermination of the gnoll brewers") . "].");

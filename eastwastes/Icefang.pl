...
sub EVENT_SIGNAL {
    #:: Match if signal from eastwastes/Korrigain.pl is "1161101".
    if ($signal == 1161101) {
        quest::moveto(3213, -8064, 146);
        $icefang = 10;
    }
    #:: Match if signal from eastwastes/Korrigain.pl is "1161102".
...
sub EVENT_WAYPOINT_DEPART {
    ...
    if ($icefang == 10) {
        ...
        #:: Send a signal "116567" to Eastern Wastes >> NAMESCRIPT (116567) with no delay.
        ...
        #:: Create a timer 'icefang' that triggers every 600 seconds (10 min 0 sec).
        quest::settimer("icefang", 600);
        $icefang = 180;

        #:: Spawn a Eastern Wastes >> ringsixcoord (116571), without grid or guild war, at the given location.
        my $entid = quest::spawn2(116571, 0, 0, -4319, -3500, 150, 0);
        #:: Get mob Eastern Wastes >> ringsixcoord ($entid).

...
sub EVENT_SAY {
    if ($text=~/hail/i) {
        ...
        #:: Give a 30265 - FINDITEM.
        quest::summonitem(30265);
    }
    elsif ($text=~/hunting/i) {
...
sub EVENT_ITEM {
    #:: Match a 30135 - Coldain Hunting Blanket.
    if (plugin::takeItems(30135 => 1)) {
		...
		#:: Give a 30131 - Copper Coldain Insignia Ring.
    ...
    #:: Match a 30267 - Broken Axe and a 30131 - Copper Coldain Insignia Ring.
    elsif (plugin::takeItems(30267 => 1, 30131 => 1)) {
		...
		#:: Give a 30133 - Silver Coldain Insignia Ring.
    ...
    #:: Match a 30137 - Coldain Hunting Knife and a 30133 - Silver Coldain Insignia Ring.
    elsif (plugin::takeItems(30137 => 1, 30133 => 1)) {
		...
		#:: Give a 30132 - Gold Coldain Insignia Ring.
    ...
    #:: Match a 30140 - Coldain Smithing Hammer and a 30132 - Gold Coldain Insignia Ring.
    elsif (plugin::takeItems(30140 => 1, 30132 => 1)) {
		...
		#:: Give a 30134 - Platinum Coldain Insignia Ring.
    ...
    #:: Match a 30141 - Head of Scarbrow and a 30134 - Platinum Coldain Insignia Ring.
    elsif (plugin::takeItems(30141 => 1, 30134 => 1)) {
    ...
    #:: Match a 1045 - Rodrick's Head a 18084 - Note from Kromrif and a 30268 - Obsidian Coldain Insignia Ring.
    elsif (plugin::takeItems(1045 => 1, 18084 => 1, 30268 => 1)) {
    ...
    #:: Match a 1047 - Note from Corbin.
    elsif (plugin::takeItems(1047 => 1)) {
		...
		#:: Give a 30163 - Adamantium Coldain Insignia Ring.
    ...
    #:: Match a 30163 - Adamantium Coldain Insignia Ring.
    elsif (plugin::takeItems(30163 => 1)) {
		quest::say("Give this to Gloradin and return to me immediately, it is time. May Brell be with us!");
		#:: Give a 1093 - Marching Orders.
...
sub EVENT_WAYPOINT_DEPART {
    if ($walk1 == undef) {
        $walk1 = $walk1 + 1;
        $walk2 = undef;
        #:: Create a timer '11' that triggers every 20 seconds (20 sec).
        ...
        $walk5 = $walk5 + 1;
        #:: Spawn a Eastern Wastes >> #Garadain_Glacierbane (116576), without grid or guild war, at the given location.
...
sub EVENT_TIMER {
    if ($timer == 11) {
        ...
        #:: Create a timer '3' that triggers every 120 seconds (2 min 0 sec).

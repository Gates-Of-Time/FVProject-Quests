...
sub EVENT_ITEM {
    #:: Match a 30265 - Dull Bladed Axe.
    if (plugin::takeItems(30265 => 1)) {
        ...
        #:: Spawn a Eastern Wastes >> Boridain_Glacierbane (116191), without grid or guild war, at the given location.
        my $mobid = quest::spawn2(116191, 0, 0, 1547, -2459, 306.5, 0);
        #:: Get mob Eastern Wastes >> Boridain_Glacierbane (116191)
    ...
    #:: Match a 30266 - Rabid Kodiak Skin.
    elsif (plugin::takeItems(30266 => 1) && $boridain == 18) {
...
sub EVENT_WAYPOINT_DEPART {
    ...
    if ($boridain == 0) {
        ...
        $npc->SetAppearance(1);
        #:: Create a timer '1' that triggers every 120 seconds (2 min).
        ...
        #:: Set appearance to Sit
        $npc->SetAppearance(1);
        #:: Spawn a Eastern Wastes >> Rabid_Tundra_Kodiak (116545), without grid or guild war, at the given location.
        my $kodiak = quest::spawn2(116545, 231, 0, 1559, -2304, 313, 125.5);
        #:: Get mob Eastern Wastes >> Rabid_Tundra_Kodiak (116545)
        my $mob = $entity_list->GetMobID($kodiak);
        my $mobnpc = $mob->CastToNPC();
        $mobnpc->MoveTo(1530, -2491, 306);
        #:: Create a timer '2' that triggers every 300 seconds (5 min).
...
sub EVENT_SIGNAL {
    $boridain = 0;
    #:: Create a timer '1' that triggers every 3 seconds (3 sec).
    quest::settimer(1, 3);
}

...
sub EVENT_SAY {
	if ($text=~/hail/i) {
		quest::say("Hail and welcome.  I am sure you have much to do, but could I ask a [" . quest::saylink("favor") . "] of you?");
	}
	if ($text=~/favor/i) {
		quest::say("Oh, um, on second thought, never mind.  I should really just do it myself.  Thank you anyway.");
	}
	if ($text=~/enchanted bow/i) {
		quest::say("Alas... Another ranger in search of the [" . quest::saylink("Rain Caller") . "]. She is no more.");
	} 	
	if ($text=~/Rain Caller/i) {
		quest::say("Rain Caller is the name we give each Trueshot longbow once it is enchanted. Unfortunately, I am unable to enchant them any longer. One of the components is no longer available. Someone would have to strike a [" . quest::saylink("deal with the fairie folk") . "]. Once I have that and the [" . quest::saylink("remaining components") . "] I can create the Rain Caller, a ranger's bow");
	}
	if ($text=~/deal with the fairie folk/i) {
		quest::say("The fairie princess, Joleena, used to have a metal gnome deliver [" . quest::saylink("fairie gold dust") . "] to the Keepers every month. She has stopped this and now refuses to offer it to any nation of Faydwer. What she is angry about, we do not know.");
	} 
	...
	if ($text=~/remaining components/i) {
		quest::say("The remaining components are the Trueshot longbow and a treant heart. There will also be the guild donation in the amount of 3000 gold coins. These and the [" . quest::saylink("fairie gold dust") . "] will merit a ranger the Rain Caller enchanted bow.");
	} 	
	if ($text=~/trades/i) {
...
sub EVENT_ITEM {	
	...
	#:: Turn in for the Enrollment Letter 18778
	...
	#:: Turn in for 12333 Pouch of Gold Dust, 12334 A Wooden Heart, 8401 Trueshot Longbow, Gold = 3000

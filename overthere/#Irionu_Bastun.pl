...
sub EVENT_ITEM {
  #:: Match a 57996 - Note to Irionu
  if (plugin::takeItems(57996 => 1)) {
    ...
    quest::set_data($key, "8");
  }
  elsif (quest::get_data($key) == 2) {
    #:: Match a 57008 - Letter from Muada
    if (plugin::takeItems(57008 =>1)) {
      quest::say("Ah, well,expect rigorousss timesss. Nice to see a familiar stroke of the pen. I learned from Muada a long time ago and it wasss not easy. His tasksss are never easy. Are you familiar with these [lands]?");
      quest::set_data($key, "3");
    }
  }
  elsif (quest::get_data($key) == 8) {
    #:: Match a 57010 - Swirling Feral Essence
    if (plugin::takeItems(57010 => 1 )) {
      ...
      #:: Give a 52900 - Sealed Note for Muada

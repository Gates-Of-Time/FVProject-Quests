$key = $name . "-" . "Fatestealer";

sub EVENT_SAY { 
	if($text=~/Hail/i){
        if(quest::get_data($key) == "1") {
		    quest::emote("shoots you an untrusting glare. He is wearing a pendant that resembles a sun occluded by the horizon.");
        }
	}	
	elsif ($text=~/the sun is setting on the horizon/i) {
        if(quest::get_data($key) == "1") {
        ...
        if(quest::get_data($key) == "1") {
		    quest::emote("winks and you see through his guise. He isn't a dark elf after all");
	...
	elsif ($text=~/change their tune/i) {
        if(quest::get_data($key) == "1") {
            ...
            #:: Give a 9590 - Notice to Cease and Desist
...
sub EVENT_ITEM {  
    if(quest::get_data($key) == "1") {
        $hasValidItemHandin = 0;
        #:: Match a 12950 - Signed Agreement (Signed with the bloody handprint of Galdorin)
        if (plugin::takeItems(10243 => 1)) {
            quest::say("Well done, $name. Galdorin had it coming, so don't feel too guilty about your part in this.");
            quest::set_data($key . "_gal", "1");
            $hasValidItemHandin = 1;
        } 
        #:: Match a 10239 - Signed Agreement (Signed by Seana Marsinger) and a 52416 - Signed Agreement (Signed by Stefan Marsinger)
        elsif (plugin::takeItems(10239 => 1, 52416 => 1)) {
            quest::say("Those Marsingers won't be speaking ill about us again anytime soon.");
            quest::set_data($key . "_mar", "1");
            $hasValidItemHandin = 1;
        }
        #:: Match a 12950 - Signed Agreement (Signed with the bloody handprint of Galdorin)
        elsif (plugin::takeItems(10243 => 1)) {
            quest::say("Excellent, Lythe has been silenced.");
            quest::set_data($key . "_lyt", "1");
            $hasValidItemHandin = 1;
        } 

        if($hasValidItemHandin) {
            if(quest::get_data($key . "_gal") == "1" && quest::get_data($key . "_mar") == "1" && quest::get_data($key . "_lyt") == "1") {

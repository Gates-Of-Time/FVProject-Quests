sub EVENT_SAY {
	if ($text=~/hail/i) {
		...
		quest::say("I have messages that need to go to - well, right now I have one that needs to go to Highpass. Will you [" . quest::saylink("deliver") . "] mail to Highpass for me?");
	}
	elsif ($text=~/mail/i) {

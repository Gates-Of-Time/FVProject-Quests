...
sub EVENT_SAY {
	if ($text=~/hail/i) {
		quest::say("Oh.. Hiya, I'm just out here fishing, since I can't find a job. I hope this [" . quest::saylink("bait") . "] I just bought catches me a big ol' fish.");
	}
	elsif ($text=~/bait/i) {
...
sub EVENT_PROXIMITY_SAY {
	if ($text=~/tacklebox/i) {
		quest::say("Oh. That mean [" . quest::saylink("dwarf") . "], Trumpy, just knocked my tacklebox into the water. Could you please get it for me? I can't swim.");	
		#:: Send a signal "1" to South Qeynos >> Trumpy_Irontoe (1042) with no delay
		quest::signalwith(1042,1,0);
	}
	elsif ($text=~/dwarf/i) {
		quest::say("His name is Trumpy. He is one of those [" . quest::saylink("Irontoes") . "] I think. I've seen him hanging out in the Fish's Ale. I don't know why he likes to torment me.");
	}
...
sub EVENT_SIGNAL {
	...
	if ($signal == 1) {
		quest::say("Oh, hi, Beren. Not too good so far. That [" . quest::saylink("dwarf") . "] keeps bothering me, too.");
...
sub EVENT_ITEM {
	...
	if (plugin::takeItems(13702 => 1)) {
		quest::say("Thank you so much!  If you want some free advice, steer clear of those [" . quest::saylink("Irontoes") . "]! They are nothing but trouble. Here, It's not much but I must thank you somehow.");

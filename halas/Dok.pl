sub EVENT_SAY {
 if($text=~/hail/i){
   ...
   quest::summonitem(17958); # Empty Jar
  }
	if($text=~/crime/i){ 
 ...
 sub EVENT_ITEM { 
 if(plugin::check_handin(\%itemcount, 12222 => 1)){ # Full Honeycomb Jar
  ...
  quest::summonitem(12220); # Everburn Candle
  ...
  if(plugin::check_handin(\%itemcount, 12275 => 1, 12276 => 1, 12282 => 1, 13953 => 1)){ # Cape of Underfoot
  quest::say("Here is your Candle o' Bravery.");
  quest::summonitem(12277); # Candle of Bravery
  quest::faction(213,3); # Merchants of Halas
  quest::faction(361,3); # Wolves of the North
  quest::faction(294,3); # Shamen of Justice
  quest::ding();

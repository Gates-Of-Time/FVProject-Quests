sub EVENT_SAY {
	if ($text=~/hail/i) {
		quest::say("Hello, $name!  It's hard to believe just how cold it gets here!  It really has been wearing me down. Especially after the [" . quest::saylink("journey") . "] from Qeynos.");
	}
	elsif ($text=~/journey/i) {
		quest::say("I have been traveling the face of Norrath to unravel the mystery of why my beloved [" . quest::saylink("brother") . "] attacked a group of traveling bards.  He didn't survive the fight.  It's been so long. I can't really remember what he looked like!  I miss him so much...  Now all I have are his [" . quest::saylink("diary") . "] and some brewing recipes he sold before his death.");
	}
		...
		quest::say("The diary?  It's one of the two connections I still have to my brother.  Sadly, [" . quest::saylink("pages") . "] are missing from it.  I've been looking for them.");
	}
	elsif ($text=~/pages/i) {

...
sub EVENT_SIGNAL {
	...
	#:: Create a scalar to store a random
		...
		if ($singasong == 1){ 
			...
			quest::signal(10199,4); #Sir Edwin Motte
			quest::signal(10012,4); #Imxil Tbrow
			}
        elsif ($singasong == 2){ 
			...
			quest::signal(10199,3); #Sir Edwin Motte
			}
		elsif ($singasong == 3){ 
			...
			quest::signal(10182,3); #Talym Shoontar
			}
		elsif ($singasong == 4){ 
			...
			quest::signal(10199,6); #Sir Edwin Motte
			}
		elsif ($singasong == 5){ 

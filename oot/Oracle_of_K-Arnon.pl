sub EVENT_SAY {
	if ($text=~/hail/i) {
		quest::say("Greetings, $name, I am the Oracle of K'Aron. Do you quest for [" . quest::saylink("knowledge") . "] yourself, or do you offer that which I [" . quest::saylink("seek") . "].");
	}
	if ($text=~/knowledge/i) {
		quest::say("I have lived long and have insight into many matters, yet I despise charity. To learn from me, you must first bring me that which I [" . quest::saylink("seek") . "].");
	}
	if ($text=~/seek/i) {
		quest::say("I seek knowledge, which is superior to all things. And no knowledge is greater than that which is contained within the [" . quest::saylink("Tome of Ages") . "].");
	}
	if ($text=~/tome of ages/i) {
		quest::say("The Tome of Ages has been compiled over the millennia by the Scribes of [" . quest::saylink("Dal") . "]. It was divided into seven distinct [" . quest::saylink("books") . "] by the scribes to keep the knowledge from one such as me. That said, with aid, I shall unite these books and the Tome of Ages shall be mine.");
	}
	if ($text=~/dal/i) {
	...
	if ($text=~/books/i) {
		quest::say("The seven books that make up the Tome of Ages are those of [" . quest::saylink("Enlightenment") . "], [" . quest::saylink("Scale") . "], [" . quest::saylink("Turmoil") . "], [" . quest::saylink("Monuments") . "], [" . quest::saylink("Elders") . "], [" . quest::saylink("Blood") . "], and the [" . quest::saylink("Lost") . "].");
	}

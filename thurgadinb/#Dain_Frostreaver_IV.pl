...
sub EVENT_SAY {
  if ($text =~ /hail/i) {
  ...
  elsif ($text =~ /Greetings and good fortune to you, honored Dain Frostreaver IV/i) {
    ...
    #:: Match if faction is better than Indifferent and Kindly or worse
    ...
    #:: Match if faction is better than Kindly
    if ($faction < 3) {
  ...
  #:: Check for 8898 - Approved Issue Kit
  elsif ($text =~ /mission/i && plugin::check_hasitem($client, 8898)) {
  ...
  #:: Check for 8898 - Approved Issue Kit
  elsif ($text =~ /factor/i && plugin::check_hasitem($client, 8898)) {
...
sub EVENT_ITEM {
  #:: Match if faction is Indifferent or better or Threateningly or worse
  #:: Match a 1500 - Box of the Guilty and a 30164 - Velium Coldain Insignia Ring
  if (($faction <= 5 || $faction >= 8) && plugin::takeItems(1500 => 1, 30164 => 1)) {
    ...
    #:: Give a 30369 - Coldain Hero's Insignia Ring
    quest::summonitem(30369);
    #:: Give a 1465 - Dirk of the Dain
    quest::summonitem(1465);
    #:: Set factions
    quest::faction(406,50);  #:: + Coldain
    quest::faction(405,50);  #:: + Dain Frostreaver IV
  ...
  #:: Match a 30516 - King Tormax's Head
  elsif ($faction == 1 && plugin::takeItems(30516 => 1)) {
    ...
    quest::say("Let it be know from this day forth that %t and their companions are Heros of the Coldain Kingdom. King Tormax has been slain, it is a time for celebration. Let no tankard go unfilled!");
    #:: Give a 30502 - Tri-Plated Golden Hackle Hammer
    quest::summonitem(30502);
    #:: Set factions
    quest::faction(406,100);   #:: + Coldain
    quest::faction(405,100);   #:: + Dain Frostreaver IV
    quest::faction(429,-100); #:: - King Tormax
  }

  #:: Match if faction is Ally
  #:: Match a 1465 - Dirk of the Dain
  elsif ($faction == 1 && plugin::takeItems(1465 => 1)) {
  ...
  #:: Match a 1199 - Runed Coldain Prayer Shawl or a 8895 - Runed Coldain Prayer Shawl (Dain approved)
    ...
    #:: Give a 8896 - Royal Coldain Orders
    quest::summonitem(8896);
  }

  #:: Match a 8886 - Coldain Standard Issue Kit
  elsif (plugin::takeItems(8886 => 1)) {
    ...
    #:: Give a 8898 - Approved Issue Kit
    quest::summonitem(8898);
    #:: Give a 8897 - Expedition Orders
    quest::summonitem(8897);
  }

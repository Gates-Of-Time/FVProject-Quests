sub EVENT_SAY { 
	if (quest::is_content_flag_enabled("Kunark_EpicsEra")) {
		...
		if ($text=~/doll/i) {
			...
			#:: Give a 20372 - Solusek Mining Company Invoice
...
sub EVENT_ITEM {
	if (quest::is_content_flag_enabled("Kunark_EpicsEra")) {
		#:: Match a 20373 - Letter of Introduction
		if (plugin::takeItems(20373 => 1)) {
		...
		#:: Match a 20371 - Note for Baenar
		elsif (plugin::takeItems(20371 => 1)) {
			...
			#:: Give a 20368 - Note to Maligar
			quest::summonitem(20368);
			#:: Ding!
			quest::ding();
		}
		#:: Match a 20367 - Maligar's Head
		elsif (plugin::takeItems(20367 => 1)) {
			...
			#:: Give a 20366 - Mahlin's Mystical Bongos

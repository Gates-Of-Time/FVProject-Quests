sub EVENT_TIMER {
	...
	if ($timer eq "return") {
		...
		$npc->NavigateTo($npc->GetSpawnPointX(), $npc->GetSpawnPointY(), $npc->GetSpawnPointZ(), $npc->GetSpawnPointH());
	}
...
sub EVENT_SAY {
	if ($text=~/hail/i) {
		...
		$npc->NavigateTo(-771, 1210, -25.50, 341);
		...
		$npc->NavigateTo(-541, 1404, -39, 507);
		...
		$npc->NavigateTo(-739, 1239, -25.5, 164);
		...
		$npc->NavigateTo(-471, 957, -28.50, 241);
		...
		$npc->NavigateTo(-845, 1299, -29.50, 126);
		...
		$npc->NavigateTo(-365, 902, -7.50, 127);
		...
		$npc->NavigateTo(-544, 1420, -78.94, 0);

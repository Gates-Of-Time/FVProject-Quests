...
sub EVENT_SAY {
	...
	#:: Under level five servants of the crown get one dialogue
	if ($text=~/serve the Crown/i && $ulevel <= 5) {
		quest::say("You are too inexperienced. Leave that business to someone more worldly."); 
	}
	#:: Over level five servants of the crown get a different dialogue
	if ($text=~/serve the Crown/i && $ulevel >= 5) {

sub EVENT_SAY {
	...
	if ($text=~/interested/i) {
		quest::say("I have messages that need to go to... well, right now I have one that needs to go to Kelethin. Will you deliver mail to [" . quest::saylink("Kelethin") . "] for me?");
	}
	if ($text=~/Kelethin/i) {

sub EVENT_SAY {
	...
	if ($text=~/interested/i) {
		quest::say("I have messages that need to go to - well, right now I have one that needs to go to Qeynos.  Will you deliver mail to [" . quest::saylink("Qeynos") . "] for me?");
	}
	if ($text=~/Qeynos/i) {

sub EVENT_SAY {
	if ($text=~/hail/i) {
		quest::say("Away from carver Cagrek you get!! Me's a busy troll. Must punish enmees of Grobb and feeds trolls a plenty. Trolls complain meats no good.  Dey says dey hears ogres have special meat which taste better den pris'ners of Grobb. Do you knows name of meat? What kinda [" . quest::saylink("hehe meat") . "] dat be? Tell me!!");
	}
...
sub EVENT_ITEM {
	...
	#:: Match three 13368 - HEHE Meat and a 18940 - Tattered Recipe
		...
		quest::givecash($cash{copper},$cash{silver},$cash{gold},$cash{platinum});
		#:: Set factions
		quest::faction(66, 20);		#:: + Da Bashers
		quest::faction(22, -10);	#:: - Broken Skull Clan
	}
	#:: Return unused items

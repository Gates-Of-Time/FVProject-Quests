sub EVENT_SAY { 
	...
	if ($class eq "Ranger) {
		if ($text=~/servant of the pine/i) {
			...
			#:: Spawn a The Plane of Sky >> Relinin_Skyrunner (71095), without grid or guild war, at the specified location
			quest::spawn2(71095, 0, 0, 617.1, 1399.5, -766.9, 129.4);
			quest::depop();
		}
		elsif ($text=~/gordon treecaller/i) {
			quest::say("I shall summon him for you then");
			#:: Spawn a The Plane of Sky >> Gordon_Treecaller (71090), without grid or guild war, at the specified location

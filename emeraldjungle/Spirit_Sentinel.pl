...
sub EVENT_ITEM {
	if ( quest::is_content_flag_enabled("Kunark_HoleEra")) {
		#:: Match if faction is Kindly or better
		if ($faction <= 3) {
			#:: Match four 1668 - Sparkling Gem
			if (plugin::takeItems(1668 => 1)) {
				...
				quest::ding();
				#:: Set factions
				quest::faction(404, 150);		#:: + Truespirit
			}
			#:: Match four 1672 - Black Dire Pelt
			elsif (plugin::takeItems(1672 => 1)) {
			...
			#:: Match four 3599 - Completed Report
			elsif (plugin::takeItems(3599 => 1)) {

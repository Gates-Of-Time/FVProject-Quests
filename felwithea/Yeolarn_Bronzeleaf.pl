...
sub EVENT_SAY {
	if ($text=~/hail/i) {
		quest::say("Hail, " .. e.other:GetName() .. "! We of Tunare are charged with protecting the Great Mother from the forces of Innoruuk. Even now, the evil minions of this foul deity are despoiling our great forest. Will you help us [" . quest::saylink("protect the Mother") . "]?");
	}
		if ($text=~/trades/i) {

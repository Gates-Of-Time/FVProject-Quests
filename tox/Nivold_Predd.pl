sub EVENT_SAY {
	if ($text=~/hail/i) {
		quest::say("What do you want? Do not disturb me lest you plan to assist in my [" . quest::saylink("summoning of Dread") . "].");
	}
	if ($text=~/summoning of dread/i) {
		quest::say("There are some components essential to the summoning of the avatar of Dread. I require the eye of a griffon, some [" . quest::saylink("powder of reanimation") . "], the brain of the Ishva Mal, and the toes of an ice giant. Fetch me these reagents so that we may summon the avatar to forge a shield worn only by the mightiest apostles of Cazic-Thule.");
	}
...
sub EVENT_ITEM {
	...
	if ($faction <= 5) {
		...
		if (plugin::check_handin(\%itemcount, 16540 => 1, 13739 => 1, 14111 => 1, 14112 => 1, )) {
			...
			quest::unique_spawn(75164,0,0,474,1230,-37,128);
		}
		else {

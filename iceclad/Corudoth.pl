sub EVENT_SAY {
	...
	elsif($text=~/halldor/i) {  #NOTE# info on allakhazam stated Petcas drops the key.  Petcas no longer drops the key with this one --  This might of changed with the warrior 1.5 epic prequest which involves Petcas.
...
sub EVENT_ITEM {
	#:: Match a 1710 - Inscribed Velium Tablet
	if (plugin::takeItems(1710 => 1)) {
		...
		#:: Give a 1711 - Lock of Dwarven Hair
		quest::summonitem(1711);
		#:: Ding!
		quest::ding();
